class DailyChatService
  ...
  def generate_daily_greeting(daily_chat)
    # 今日の日付情報を取得
    today = daily_chat.chat_date
    day_info = get_day_information(today)

    # 曜日と時間帯に応じた挨拶
    greeting_prompt = build_greeting_prompt(today, day_info)
  ...
  def build_greeting_prompt(date, day_info)
    ...
    <<~TEXT
      ...
      　例：9月23日なら「今日は秋分の日でしたね。秋を感じる出来事はありましたか？」等
